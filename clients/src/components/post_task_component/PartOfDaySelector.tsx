...
const PartOfDaySelector = ({ PartOfDaySelect }: any) => {
  const [selected, setSelected] = useState<string>();
  // HANDLE SELECTION DAY
  const handleSelection = (items: any) => {
    setSelected(items.part);
    PartOfDaySelect(items);
  };
  const EveningIcon: React.FC = () => {
    return (
      <svg
        height="32"
        width="32"
  ...
  const data = [
    { part: "Evening", time: "After 6pm", Icon: <EveningIcon /> },
    { part: "Afternoon", time: "2pm - 6pm", Icon: <AfternonIcon /> },
    { part: "Midday", time: "10am - 2pm", Icon: <MiddayIcon /> },
    { part: "Morning", time: "Before 10am", Icon: <MorningIcon /> },
  ];

  return (
    <div className="d-flex gap-3 d-lg-flex d-sm-block d-md-block">
      {data.reverse().map((items, index) => (
        <div
          key={index}
          className={`${selected == items.part && "selected"} part_of_day_card`}
          onClick={() => handleSelection(items)}
        >
          {items.Icon}
          <Typography style={styles.itemText}>{items.part}</Typography>
          <Typography style={styles.itemText}>{items.time}</Typography>
        </div>
      ))}
    </div>
  );
